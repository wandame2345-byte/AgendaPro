...
function renderNav(){
  ...
  if(navElem){
    navElem.innerHTML=mainItems.map(it=>`<button data-section="${it.id}">${it.label}</button>`).join('');
  ...
  if(mainItems.length > 0) showSection(mainItems[0].id);
}
...
async function refreshData(){
  try{
    [procedures,clients,appointments]=await Promise.all([api('/api/procedures'),api('/api/clients'),api('/api/appointments')]);
...
/* =========================
   FILTRO DE PERÍODO — DASHBOARD (PROTEGIDO CONTRA NULL)
   ========================= */
...
function applyDashboardFilter(){
  const startEl = $('dashFilterStart') || $('startDate') \vert{}\vert{}$('filterStartDate');
  const endEl = $('dashFilterEnd') || $('endDate') \vert{}\vert{}$('filterEndDate');
...
/* =========================
   FILTRO DE PERÍODO — AGENDA (PROTEGIDO CONTRA NULL)
   ========================= */
function applyAgendaFilter(){
  const startEl = $('agendaFilterStart') \vert{}\vert{}$('filterAgendaStart');
  const endEl = $('agendaFilterEnd') \vert{}\vert{}$('filterAgendaEnd');
...
function renderAgenda(){
  ...
  if($('slots'))$('slots').innerHTML=hours.map(h=>{
    ...
    return a?`<div class="slot busy"><strong>${h}</strong>${esc(a.name)}<small>${esc(a.procedure)} • ${money(a.price)}</small></div>`:`<div class="slot free" onclick="openModal('${d}','${h}')"><strong>${h}</strong>Disponível<small>Clique para agendar</small></div>`;
  }).join('');
  ...
  if($('dayTable'))$('dayTable').innerHTML=list.length?`<table><tr><th>Hora</th><th>Cliente</th><th>Foto</th><th>Procedimento</th><th>Valor</th><th>Status</th><th>Obs.</th><th>Ações</th></tr>${list.map(a=>{
    ...
    return `<tr><td><b>${a.time}</b></td><td>${esc(a.name)}<br><small>${esc(a.phone)}</small></td><td>${c?.photo?`<img src="${esc(c.photo)}" class="client-photo">`:'<small style="color:var(--muted)">Sem foto</small>'}</td><td>${esc(a.procedure)}</td><td>${money(a.price)}</td><td><span class="badge ${badge(a.status)}">${a.status}</span></td><td>${esc(a.note||'-')}</td><td><button type="button" class="btn success btn-sm" onclick="setStatus(${a.id},'Atendido')">Concluído</button><button type="button" class="btn danger btn-sm" onclick="setStatus(${a.id},'Cancelado')">Não Concluído</button></td></tr>`;
...
if($('clientPhoto')){$('clientPhoto').addEventListener('change',()=>{
    ...
    r.onload=e=>{ if($('photoPreview')) { $('photoPreview').src=e.target.result; $('photoPreview').style.display='block'; } };
    r.readAsDataURL(f);
  });
}
...
async function addProcedure(){
  const name=$('procName')?.value.trim()\vert{}\vert{}'', price=Number($('procPrice')?.value)||0;
  if(!name)return alert('Informe o nome do procedimento.');
  try{
    await api('/api/procedures',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,price})});
...
async function changePassword(e){
  ...
  if(newPassword!==confirmPassword){
    if(message) message.textContent='A nova senha e a confirmação não são iguais.';
    return;
  }

  if(newPassword.length<6){
    if(message) message.textContent='A nova senha deve ter pelo menos 6 caracteres.';
    return;
  }

  if(currentPassword===newPassword){
    if(message) message.textContent='A nova senha deve ser diferente da senha atual.';
    return;
  }
...
if($('cbPhoto')){$('cbPhoto').addEventListener('change',()=>{
    ...
    if(!f)return;
    const r=new FileReader();
    r.onload=e=>{ if($('cbPhotoPreview')) { $('cbPhotoPreview').src=e.target.result; $('cbPhotoPreview').style.display='block'; } };
    r.readAsDataURL(f);
  });
}
