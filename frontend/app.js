...
if($('loginForm')) {$('loginForm').addEventListener('submit',async e=>{
   e.preventDefault();
   try{
     const data=await api('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:roleEmail(pendingRole),password:$('loginPassword')?.value,role:pendingRole})});
...
function fillTimes(){if($('time'))$('time').innerHTML='<option value="">Selecione</option>'+hours.map(h=>`<option>${h}</option>`).join('')}
function openModal(date=localDate(),time=''){if($('modal'))$('modal').classList.add('show');if($('date'))$('date').value=date;if($('time'))$('time').value=time;fillAllProcedureSelects()}
function closeModal(){if($('modal'))$('modal').classList.remove('show');if($('appointmentForm'))$('appointmentForm').reset();if($('date'))$('date').value=localDate()}

if($('appointmentForm')) {$('appointmentForm').addEventListener('submit',async e=>{
   e.preventDefault();
   try{
     await api('/api/appointments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:$('name').value.trim(),phone:$('phone').value.trim(),date:$('date').value,time:$('time').value,procedureId:$('procedure').value,price:Number($('price').value),status:$('status').value,note:$('note').value.trim()})});
     closeModal();
     await refreshData();
...
function renderDashboard(){
 const t=localDate(),w=periodTotals(startWeek(t),endWeek(t)),m=periodTotals(t.slice(0,7)+'-01',new Date(new Date(t+'T12:00:00').getFullYear(),new Date(t+'T12:00:00').getMonth()+1,0).toISOString().slice(0,10)),day=periodTotals(t,t);
...
function changeDay(n){let d=new Date(($('agendaDate')?.value||localDate())+'T12:00:00');d.setDate(d.getDate()+n);if($('agendaDate'))$('agendaDate').value=d.toISOString().slice(0,10);renderAgenda()}
function goToday(){if($('agendaDate'))$('agendaDate').value=localDate();renderAgenda()}
function openClientModal(){if($('clientModal'))$('clientModal').classList.add('show')}
function closeClientModal(){if($('clientModal')) $('clientModal').classList.remove('show');if($('clientForm')) $('clientForm').reset();if($('photoPreview')) { $('photoPreview').style.display='none';$('photoPreview').src=''; }}
...
async function renderReports(){
 ...
 try{
   ...
   const map={};
   r.data.forEach(a=>map[a.procedimento]=(map[a.procedimento]||0)+1);
